...
RSpec.describe 'Post' do
  ...
  describe 'GET /api/posts/search/gatto' do
    it 'finds posts by title or tag' do
      _posts = [
        Post.create!(title: 'Post su Gatto Persiano', tags: [Tag.new(name: 'gatto'), Tag.new(name: 'animale')]),
        Post.create!(title: 'Le arance fanno bene', tags: [Tag.new(name: 'banana'), Tag.new(name: 'frutto')]),
        Post.create!(title: 'La Tigre della Malesia',
                     tags: [Tag.new(name: 'tigre'), Tag.new(name: 'gatto'), Tag.new(name: 'animale')])
      ]
      term = 'gatto'
      ...
      expect(json[1]['tags']).to include(include({ 'name' => term }))
    end
  end
end
